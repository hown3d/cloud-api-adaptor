...
docker() {
	...
	[[ "${DOCKER_PODVM_IMAGE}" ]] && optionals+="-podvm-docker-image ${DOCKER_PODVM_IMAGE} "sta
...
stackit() {
	test_vars STACKIT_PROJECT_ID STACKIT_NETWORK_ID STACKIT_AVAILABILITY_ZONE STACKIT_SECURITY_GROUP PODVM_IMAGE_NAME
	[[ "${STACKIT_REGION}" ]] && optionals+="-stackit-region ${STACKIT_REGION} "
	[[ "${STACKIT_IAAS_URL}" ]] && optionals+="-stackit-iaas-endpoint-url ${STACKIT_IAAS_URL} "
	[[ "${STACKIT_MACHINE_TYPE}" ]] && optionals+="-stackit-machine-type ${STACKIT_MACHINE_TYPE} "
	[[ "${STACKIT_KEYPAIR}" ]] && optionals+="-stackit-key-pair ${STACKIT_KEYPAIR} "
	[[ "${STACKIT_DEDICATED_INSTANCES}" ]] && optionals+="-stackit-dediciated-instances ${STACKIT_INSTANCES} "

	set -x
	exec cloud-api-adaptor stackit \
		-pods-dir "${PEER_PODS_DIR}" \
		-socket "${REMOTE_HYPERVISOR_ENDPOINT}" \
		-stackit-image "${PODVM_IMAGE_NAME}" \
		-stackit-network-id "${STACKIT_NETWORK_ID}" \
		-stackit-project-id "${STACKIT_PROJECT_ID}" \
		-stackit-availability-zone "${STACKIT_AVAILABILITY_ZONE}" \
		-stackit-security-group "${STACKIT_SECURITY_GROUP}" \
		${optionals}
}
...
help_msg() {
	cat <<EOF
Usage:
	CLOUD_PROVIDER=alibabacloud|aws|azure|byom|gcp|ibmcloud|ibmcloud-powervs|libvirt|vsphere|docker|stackit $0
or
	$0 alibabacloud|aws|azure|byom|gcp|ibmcloud|ibmcloud-powervs|libvirt|vsphere|docker|stackit
...
if [[ "$CLOUD_PROVIDER" == "aws" ]]; then
	...
	docker
elif [[ "$CLOUD_PROVIDER" == "stackit" ]]; then
	stackit
else
	help_msg
fi
